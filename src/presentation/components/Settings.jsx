...
function Settings({ settings, onUpdateSettings, showToast, success, error, warning }) {
  ...
  return (
    <div className="page-section active">
      ...
      <div className="card mt-2">
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginTop: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <button className="btn btn-warning" onClick={handleRestoreBackup} disabled={loading}>
              ⚠️ استعادة
            </button>
          </div>
        </div>
        ...
        {stats.lastBackup && (
          <div style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: 'var(--gray-500)' }}>
            📅 آخر نسخة: {formatDate(stats.lastBackup.date)}
          </div>
        )}
      </div>
      ...
      {/* ============================================================ */}
      <div className="card mt-2">
        <div className="card-title">📄 إعدادات الصفحات</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.5rem' }}>
          <label>
            <input
              type="checkbox"
              checked={appSettings.showInvoices}
              onChange={(e) => handleSettingChange('showInvoices', e.target.checked)}
              style={{ marginLeft: '0.5rem' }}
            />
            🧾 الفواتير
          </label>
          <label>
            <input
              type="checkbox"
              checked={appSettings.showPayments}
              onChange={(e) => handleSettingChange('showPayments', e.target.checked)}
              style={{ marginLeft: '0.5rem' }}
            />
            💵 الدفعات
          </label>
          <label>
            <input
              type="checkbox"
              checked={appSettings.showDebts}
              onChange={(e) => handleSettingChange('showDebts', e.target.checked)}
              style={{ marginLeft: '0.5rem' }}
            />
            📋 الديون
          </label>
          <label>
            <input
              type="checkbox"
              checked={appSettings.showExpenses}
              onChange={(e) => handleSettingChange('showExpenses', e.target.checked)}
              style={{ marginLeft: '0.5rem' }}
            />
            💸 المصروفات
          </label>
          <label>
            <input
              ...
              onChange={(e) => handleSettingChange('showReports', e.target.checked)}
              style={{ marginLeft: '0.5rem' }}
            />
            📈 التقارير
          </label>
      ...
      <div className="card mt-2">
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan="5" className="text-center">لا يوجد مستخدمين</td></tr>
              ) : (
                users.map(u => {
                    ...
                    <tr key={u.id}>
                      ...
                      <td>
                        <span className="badge-status badge-primary">
                          {u.role === 'admin' ? 'مدير' : u.role === 'manager' ? 'مدير تنفيذي' : 'مستخدم'}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>
                        {!isCurrentUser && (
                          <button className="btn btn-danger btn-xs" onClick={() => handleDeleteUser(u.id)}>
                            🗑️ حذف
                          </button>
                        )}
                        {isCurrentUser && <span className="text-muted">(حسابك الحالي)</span>}
                      </td>
                    </tr>
                  );
                })
